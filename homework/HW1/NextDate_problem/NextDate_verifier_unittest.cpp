...
/*
// weak normal
TEST(NextDateVerifierEquivalenceClassTest, WeakNormal) {
  EXPECT_EQ("Equilateral", NextDate_verifier(5, 5, 5));
  EXPECT_EQ("Isosceles", NextDate_verifier(50, 80, 80));
  EXPECT_EQ("Scalene", NextDate_verifier(3, 4, 5));
  EXPECT_EQ("Not a triangle", NextDate_verifier(1, 2, 3));
}
...
// weak robust
TEST(NextDateVerifierEquivalenceClassTest, WeakRobust) {
  EXPECT_EQ("Invalid input", NextDate_verifier(-1, 5, 5));
  EXPECT_EQ("Invalid input", NextDate_verifier(5, -1, 5));
  EXPECT_EQ("Invalid input", NextDate_verifier(5, 5, -1));
  EXPECT_EQ("Invalid input", NextDate_verifier(201, 5, 5));
  EXPECT_EQ("Invalid input", NextDate_verifier(5, 201, 5));
  EXPECT_EQ("Invalid input", NextDate_verifier(5, 5, 201));
}

// strong robust
TEST(NextDateVerifierEquivalenceClassTest, StrongRobust) {
  EXPECT_EQ("Invalid input", NextDate_verifier(-1, 5, 5));
  EXPECT_EQ("Invalid input", NextDate_verifier(5, -1, 5));
  EXPECT_EQ("Invalid input", NextDate_verifier(5, 5, -1));
  EXPECT_EQ("Invalid input", NextDate_verifier(-1, -1, 5));
  EXPECT_EQ("Invalid input", NextDate_verifier(-1, 5, -1));
  EXPECT_EQ("Invalid input", NextDate_verifier(-1, -1, -1));
}
